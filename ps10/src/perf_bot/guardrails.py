"""Guardrails for Performance Bottleneck Bot — input/output validation, PII masking, prompt injection detection."""
...
def mask_pii(text: str) -> str:
    for pii_type, pattern in _PII_PATTERNS.items():
        if pii_type == "ip_address":
            text = pattern.sub(
                lambda m: _PII_MASKS[pii_type] if not _is_likely_version(m.group()) else m.group(),
                text,
            )
        else:
