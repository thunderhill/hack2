"""Guardrails for Manufacturing Quality Inspection — input/output validation, PII masking, prompt injection detection."""
...
_VALID_QUALITY_STATUSES = {"PASS", "FAIL", "CONDITIONAL_PASS"}
_VALID_DISPOSITIONS = {"rework", "scrap", "accept", "quarantine"}
...
def validate_output(result) -> tuple[bool, list[str]]:
    ...
    if hasattr(result, "overall_quality_status"):
        if result.overall_quality_status not in _VALID_QUALITY_STATUSES:
            issues.append(f"Invalid overall_quality_status '{result.overall_quality_status}' — expected one of {_VALID_QUALITY_STATUSES}")

    if hasattr(result, "quality_score"):
        if not (0.0 <= result.quality_score <= 100.0):
            issues.append(f"quality_score {result.quality_score} out of range — expected 0.0 to 100.0")

    if hasattr(result, "defect_count") and hasattr(result, "defects"):
        if result.defect_count != len(result.defects):
            issues.append(f"defect_count ({result.defect_count}) does not match len(defects) ({len(result.defects)})")

    if hasattr(result, "disposition"):
        if result.disposition.lower() not in _VALID_DISPOSITIONS:
            issues.append(f"Invalid disposition '{result.disposition}' — expected one of {_VALID_DISPOSITIONS}")

    if hasattr(result, "corrective_actions"):
        if not result.corrective_actions:
            issues.append("No corrective actions provided")

    if hasattr(result, "root_cause_analysis"):
        if not result.root_cause_analysis.strip():
            issues.append("Root cause analysis is empty")
