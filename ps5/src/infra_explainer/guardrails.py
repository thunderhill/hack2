"""Guardrails for Infrastructure Change Explainer — input/output validation, PII masking, prompt injection detection."""
...
_VALID_CHANGE_TYPES = {"terraform", "kubernetes", "ansible", "cloudformation", "firewall", "network", "other"}
_VALID_RISK_LEVELS = {"critical", "high", "medium", "low"}
_VALID_CHANGE_NATURES = {"additive", "destructive", "modifying", "scaling", "security", "configuration"}
_VALID_APPROVAL_RECOMMENDATIONS = {"APPROVE", "APPROVE_WITH_CONDITIONS", "REJECT", "NEEDS_MORE_INFO"}
...
def validate_output(result) -> tuple[bool, list[str]]:
    ...
    if hasattr(result, "change_type"):
        if result.change_type.lower() not in _VALID_CHANGE_TYPES:
            issues.append(f"Invalid change_type '{result.change_type}' — expected one of {_VALID_CHANGE_TYPES}")

    if hasattr(result, "risk_level"):
        if result.risk_level.lower() not in _VALID_RISK_LEVELS:
            issues.append(f"Invalid risk_level '{result.risk_level}' — expected one of {_VALID_RISK_LEVELS}")

    if hasattr(result, "change_nature"):
        if result.change_nature.lower() not in _VALID_CHANGE_NATURES:
            issues.append(f"Invalid change_nature '{result.change_nature}' — expected one of {_VALID_CHANGE_NATURES}")

    if hasattr(result, "approval_recommendation"):
        if result.approval_recommendation.upper() not in _VALID_APPROVAL_RECOMMENDATIONS:
            issues.append(f"Invalid approval_recommendation '{result.approval_recommendation}' — expected one of {_VALID_APPROVAL_RECOMMENDATIONS}")

    if hasattr(result, "summary"):
        if not result.summary.strip():
            issues.append("Summary is empty")

    if hasattr(result, "plain_english"):
        if not result.plain_english.strip():
            issues.append("Plain English explanation is empty")

    if hasattr(result, "review_checklist"):
        if not result.review_checklist:
            issues.append("Review checklist is empty")
