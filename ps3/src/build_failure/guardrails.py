"""Guardrails for Build Failure Diagnosis — input/output validation, PII masking, prompt injection detection."""
...
_VALID_ERROR_TYPES = {"compilation", "dependency", "configuration", "linking", "syntax", "runtime", "other"}
...
def validate_output(result) -> tuple[bool, list[str]]:
    ...
    if hasattr(result, "build_tool"):
        if not result.build_tool.strip():
            issues.append("Build tool is empty")

    if hasattr(result, "language"):
        if not result.language.strip():
            issues.append("Language is empty")

    if hasattr(result, "error_type"):
        if result.error_type.lower() not in _VALID_ERROR_TYPES:
            issues.append(f"Invalid error_type '{result.error_type}' — expected one of {_VALID_ERROR_TYPES}")

    if hasattr(result, "diagnosis"):
        if not result.diagnosis.strip():
            issues.append("Diagnosis is empty")

    if hasattr(result, "fix_explanation"):
        if not result.fix_explanation.strip():
            issues.append("Fix explanation is empty")

    if hasattr(result, "code_fix"):
        if not result.code_fix.strip():
            issues.append("Code fix is empty")
