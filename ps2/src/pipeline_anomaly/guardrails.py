"""Guardrails for Pipeline Anomaly Explainer — input/output validation, PII masking, prompt injection detection."""
...
def validate_output(result) -> tuple[bool, list[str]]:
    ...
    if hasattr(result, "severity"):
        if result.severity.lower() not in _VALID_SEVERITIES:
            issues.append(f"Invalid severity '{result.severity}' — expected one of {_VALID_SEVERITIES}")

    if hasattr(result, "remediation_steps"):
        if not result.remediation_steps:
            issues.append("No remediation steps provided")

    if hasattr(result, "prevention_tips"):
        if not result.prevention_tips:
            issues.append("No prevention tips provided")

    if hasattr(result, "anomaly_type"):
        if not result.anomaly_type.strip():
            issues.append("Anomaly type is empty")
