"""Guardrails for Travel Expense Report Summarizer — input/output validation, PII masking, prompt injection detection."""
...
_VALID_RECOMMENDATIONS = {"APPROVE_FULL", "APPROVE_PARTIAL", "REJECT", "NEEDS_MORE_INFO"}
...
def validate_output(result) -> tuple[bool, list[str]]:
    ...
    if hasattr(result, "approval_recommendation"):
        if result.approval_recommendation not in _VALID_RECOMMENDATIONS:
            issues.append(f"Invalid approval_recommendation '{result.approval_recommendation}' — expected one of {_VALID_RECOMMENDATIONS}")

    if hasattr(result, "total_claimed"):
        if result.total_claimed < 0:
            issues.append(f"total_claimed is negative: {result.total_claimed}")

    if hasattr(result, "total_approved"):
        if result.total_approved < 0:
            issues.append(f"total_approved is negative: {result.total_approved}")

    if hasattr(result, "total_claimed") and hasattr(result, "total_approved"):
        if result.total_approved > result.total_claimed:
            issues.append(f"total_approved ({result.total_approved}) exceeds total_claimed ({result.total_claimed})")

    if hasattr(result, "employee_name"):
        if not result.employee_name.strip():
            issues.append("Employee name is empty")

    if hasattr(result, "trip_purpose"):
        if not result.trip_purpose.strip():
            issues.append("Trip purpose is empty")

    if hasattr(result, "items"):
        if not result.items:
            issues.append("No expense items provided")

    if hasattr(result, "approval_notes"):
        if not result.approval_notes.strip():
            issues.append("Approval notes are empty")
