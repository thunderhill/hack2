"""Guardrails for Capacity Planning Advisor — input/output validation, PII masking, prompt injection detection."""
...
_VALID_SCALING_STRATEGIES = {"vertical", "horizontal", "hybrid", "serverless"}
...
def validate_output(result) -> tuple[bool, list[str]]:
    ...
    if hasattr(result, "capacity_risk_level"):
        if result.capacity_risk_level.lower() not in _VALID_RISK_LEVELS:
            issues.append(f"Invalid capacity_risk_level '{result.capacity_risk_level}' — expected one of {_VALID_RISK_LEVELS}")

    if hasattr(result, "scaling_strategy"):
        if result.scaling_strategy.lower() not in _VALID_SCALING_STRATEGIES:
            issues.append(f"Invalid scaling_strategy '{result.scaling_strategy}' — expected one of {_VALID_SCALING_STRATEGIES}")

    if hasattr(result, "bottlenecks"):
        if not result.bottlenecks:
            issues.append("No bottlenecks identified")
    ...
    if hasattr(result, "total_cost_estimate"):
        if not result.total_cost_estimate.strip():
            issues.append("Total cost estimate is empty")
