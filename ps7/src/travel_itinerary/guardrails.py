"""Guardrails for Travel Itinerary Generator — input/output validation, PII masking, prompt injection detection."""
...
# ── Travel-specific Input Validation ─────────────────────────────────────────

_VALID_BUDGETS = {"budget", "mid-range", "luxury"}


def validate_travel_input(destination: str, duration_days: int, budget: str) -> tuple[bool, list[str]]:
    """Validate travel-specific inputs. Returns (is_valid, list_of_issues)."""
    issues = []
    ...
    if not isinstance(duration_days, int) or duration_days < 1 or duration_days > 30:
        issues.append(f"Duration must be between 1 and 30 days, got {duration_days}")

    if budget.lower() not in _VALID_BUDGETS:
        issues.append(f"Invalid budget '{budget}' — expected one of {_VALID_BUDGETS}")
...
_VALID_TRAVEL_STYLES = {"budget", "mid-range", "luxury"}
...
def validate_output(result) -> tuple[bool, list[str]]:
    ...
    if hasattr(result, "destination"):
        if not result.destination or not result.destination.strip():
            issues.append("Destination is empty")

    if hasattr(result, "travel_style"):
        if result.travel_style.lower() not in _VALID_TRAVEL_STYLES:
            issues.append(f"Invalid travel_style '{result.travel_style}' — expected one of {_VALID_TRAVEL_STYLES}")

    if hasattr(result, "days"):
        if not result.days:
            issues.append("No days provided in itinerary")

    if hasattr(result, "total_budget_estimate"):
        if not result.total_budget_estimate or not result.total_budget_estimate.strip():
            issues.append("Total budget estimate is empty")

    if hasattr(result, "packing_essentials"):
        if not result.packing_essentials:
            issues.append("No packing essentials provided")

    if hasattr(result, "cultural_tips"):
        if not result.cultural_tips:
            issues.append("No cultural tips provided")
